...
![SSH config backup created](screenshots/ssh-config-backup.png)
...
![SSH ED25519 key pair generated](screenshots/ssh-keygen.png)
...
> **Do not proceed to step 5 until this test succeeds.** Disabling password authentication before confirming key-based login works risks permanent lockout, especially on a remote server without console/VM access.
...
Both files must be edited because Ubuntu's cloud-init generates its own SSH override file (`50-cloud-init.conf`) that takes precedence over the main `sshd_config`. Editing only `sshd_config` can result in password authentication silently re-enabling.
...
![SSH configuration validated and restarted](screenshots/ssh-config-validated.png)
...
![Password auth rejected, key auth succeeds](screenshots/ssh-final-test.png)
